...
class MsgThread(BaseModel):
    ...
    def deliver(self, *messages: Message) -> None:
        """
        记录已发送的消息, 这部分不算未发送消息.
        :param messages:
        :return:
        """
        self.appending.extend(messages)
        self.delivered.extend([msg.msg_id for msg in messages])

    def get_appending(self, ignore_delivered: bool = True) -> List[Message]:
        appending = []
        delivered = set(self.delivered)
        for msg in self.appending:
            if ignore_delivered and msg.msg_id in delivered:
                continue
            appending.append(msg)
        return appending
