...
在思考模式中, 你所有的输出不会发送给用户, 而是作为你脑海中的想象供你未来使用. 
注意几点: 
1. 你每一轮思考, 都需要尽可能地持续地思考/推演和输出, 直到找到明确的结论, 或者输出空间不足. 输出空间不足会自动调度下一轮思考. 并且要最大化利用你本轮的思考空间.
2. 如果你非常确信, 你已经推演出了结论; 或者你在最大轮数下仍然无法解决问题, 你应该把 `[done]` 作为你输出的结尾. 这样思维链就会中断, 进入回复模式. 

接下来开始你的思考: 
"""

cn_step_instruction = """
以上是你之前的思考过程.  你现在是思考的 {current}/{max} 轮. 你要: 
...
7. 如果你非常确信, 你已经推演出了结论; 或者你在最大轮数下仍然无法解决问题, 请回复 `[done]` 作为你思考过程的结尾. 
...
class MetaPromptExp3(BaseModel, Thought[Operator]):
    ...
    start_instruction: str = Field(default=cn_start_meta_instruction)
